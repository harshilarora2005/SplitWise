...
export default function CreateGroupModal({ onClose, onCreated }) {
    ...
    return (
        ...
        >
        <div className="bg-[#16181c] border border-[#363a45] rounded-2xl p-8 w-full max-w-md shadow-2xl animate-[slideUp_0.2s_ease]">
            <div className="mb-6">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <input placeholder="e.g. Goa Trip" value={name} onChange={e => setName(e.target.value)} className={inputClass} />
            </div>
            ...
            {error && (
                <div className="text-sm text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-3.5 py-2.5">{error}</div>
            )}

            <div className="flex justify-end gap-2.5 mt-1">
                <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-[#9a9da8] border border-[#2a2d35] rounded-lg hover:text-white hover:bg-[#1e2026] transition-all">
                Cancel
                </button>
                <button type="submit" disabled={loading} className="px-4 py-2 text-sm bg-emerald-400 hover:bg-emerald-300 text-[#0a1a13] font-semibold rounded-lg transition-all disabled:opacity-40">
