...
export default function Dashboard() {
    ...
    const StatCard = ({ value, label, sub, accent }) => (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] px-5 py-4">
            <p className={`font-serif text-2xl font-normal mb-0.5 ${accent || 'text-white'}`}>
                {value}
            </p>
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-10 py-10">
            ...
            <div className="mb-10">
                ...
                <h1 className="font-serif text-2xl sm:text-3xl text-white font-normal leading-snug">
                    Good to see you,{' '}
                    <em className="not-italic text-emerald-400">{user?.name}</em>
                </h1>

                <p className="text-[13px] text-white/30 mt-1.5">
                    Here's a summary across all your groups
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
                ...
                <StatCard
                    ...
                    label="Expenses"
                    sub={avgPerExpense > 0 ? `avg ₹${avgPerExpense.toFixed(0)} each` : null}
                />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6">
                    <p className="text-xs font-medium text-white/25 uppercase tracking-widest mb-5">
                        Spend by category
                    </p>
                    ...
                    {recentExpenses.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-2">
                            {recentExpenses.map(expense => (
                                ...
                                >
                                    <div
                                        className={`w-8 h-8 rounded-lg border flex items-center justify-center text-xs shrink-0 ${
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
                        {groups.slice(0, 4).map(group => (
                            ...
                            >
                                ...
                                <span className="text-white/20 group-hover:text-white/50 text-sm transition-colors">
                                    →
                                </span>
                            </Link>
                        ))}
